...
"""Plot a map including spawn point and junction information."""
...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_point(surface, p, color=COLOR_ALUMINIUM_2):
            """ """
            if isinstance(p, carla.Transform):
                p = p.location
            pygame.draw.circle(surface, color,
                    world_to_pixel(p),
                    world_to_pixel_width(1), width=0)
        
        def draw_circle(surface, p, radius, color=COLOR_ALUMINIUM_2):
            if isinstance(p, carla.Transform):
                p = p.location
            pygame.draw.circle(surface, color,
                    world_to_pixel(p),
        ...
        def draw_label(surface, transform, label, color=COLOR_ALUMINIUM_2):
            """ """
            font_surface = self._label_font.render(str(label), False, color)
        ...
        def draw_topology_again(topology):
            # get nodes in graph
            ...
            #     pt = wp.transform
            #     if wp.is_junction:
            #         draw_point(map_surface, pt, color=COLOR_ORANGE_0)
            #     else:
            #         draw_point(map_surface, pt, color=COLOR_SKY_BLUE_0)
            # tag a junction as controlled or uncontrolled
            ...
            #     draw_circle(map_surface, pt, world_to_pixel_width(35), color=COLOR_SCARLET_RED_0)
            #     draw_point(map_surface, pt, color=COLOR_SCARLET_RED_0)
            # for junction in uncontrolled_junctions:
            #     pt = junction.bounding_box.location
            #     draw_circle(map_surface, pt, world_to_pixel_width(35), color=COLOR_CHAMELEON_0)
        ...
        # get data about graph topology
        draw_topology_again(topology)

        # get data about spawn points
        for idx, sp in enumerate(carla_map.get_spawn_points()):
            spp = carla.Transform(
                    sp.location - 2.0 * sp.get_forward_vector(),
                    sp.rotation)
            draw_label(map_surface, spp, idx, color=COLOR_CHOCOLATE_0)
            draw_arrow(map_surface, sp, color=COLOR_CHOCOLATE_0)

        # show all landmarks
        # for idx, lm in enumerate(carla_map.get_all_landmarks()):
        #     draw_label(map_surface, lm.transform, lm.name,
        #             color=COLOR_SCARLET_RED_0)

        # get data about stucture
